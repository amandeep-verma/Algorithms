 ...
 *  from end and find a appropriate place for it. This is how we sort the whole array by adding an element with each iteration to the 
 *  sorted array to have the output as whole sorted array.
 */
	...
	private static int[] ISort(int[] sample) {
		...
		for(int i=1;i<sample.length;i++)
		{
			...
			for(j=i;j>0;j--)
			{
				...
				{
					break; //break the loop if swapping didn't happen because that means number has made to the int right position
				}
			}
